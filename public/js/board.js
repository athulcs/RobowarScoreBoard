...
socket.on('initial game state', function (initialState) {
	...
	if (FIRST_CONFIG) {
		...
		var homehtml = '<h4 class="teamlabel" id="team1">' + initialConfig.team_home + '</h4><h2 class="scorenumber" id="' + initialConfig.team_home + 'score"></h2>';
		var awayhtml = '<h4 class="teamlabel" id="team2">' + initialConfig.team_away + '</h4><h2 class="scorenumber" id="' + initialConfig.team_away + 'score"></h2>';
...
//
socket.on('reset board',function (newTeam){
		resetGame(newTeam);
});
...
//Function to reset team name

function resetGame(newTeam){
	$('#teaml').text(newTeam.team_home);
	$('#team2').text(newTeam.team_away);
	setScore(newTeam.team_home,0);
	setScore(newTeam.team_away,0);
	stopClock();
	CURRENT_TIME = initialConfig.half_length;
	$('#mainclock').css('color', 'black');
	updateClock();
